...
def tellinput(paraml, input=None, db=None):
    ...
    if tells:
        user_from, message, _time, chan = tells[0]
        _time = datetime.fromtimestamp(int(float(_time)))
        reltime = timesince.timesince(_time)
...
@hook.command
def tellmany(inp, nick='', chan='', db=None, conn=None):
    ".tellmany <nick> <nick>, <message> -- relay <message> to <nick> when <nick> is around"

    msg = inp.split(',', 1)
    if len(msg) != 2:
        return tellmany.__doc__
    nicks = msg[0].lower().split(' ')
    message = msg[1].strip()
    user_from = nick
    
    for user_to in nicks:
